...
*Table generated by scripts/generate_paper_tables.py — see [benchmark_table.md](file:///paper/tables/benchmark_table.md).*
...
- **Data Availability**: All benchmark and case study evaluation datasets are synthetically generated during runtime via `benchmarks/dgps.py` with empirical parameters calibrated to reference datasets (documented in `data/DATASHEET.md`). No proprietary or restricted individual-level data are required; all generators are fully deterministic given a random seed.
