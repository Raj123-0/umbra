...
def _compute_imr_observed(eta: np.ndarray) -> np.ndarray:
    ...
    lambda_1(eta) = phi(eta) / Phi(eta)
    """
    eta = np.clip(eta, -30.0, 30.0)
    phi = stats.norm.pdf(eta)
    Phi = stats.norm.cdf(eta)
    # For very negative eta, use log-scale: exp(logpdf - logcdf)
    safe_Phi = np.where(Phi < 1e-12, 1e-12, Phi)
    imr = phi / safe_Phi
    # Fallback for extreme tails
    extreme_mask = eta < -10.0
    if np.any(extreme_mask):
        imr[extreme_mask] = np.exp(
            stats.norm.logpdf(eta[extreme_mask]) - stats.norm.logcdf(eta[extreme_mask])
        )
...
def _compute_imr_missing(eta: np.ndarray) -> np.ndarray:
    ...
    lambda_0(eta) = -phi(eta) / (1 - Phi(eta)) = -phi(-eta) / Phi(-eta)
    Always negative.
    """
    eta = np.clip(eta, -30.0, 30.0)
    neg_eta = -eta
    ...
    extreme_mask = neg_eta < -10.0
    if np.any(extreme_mask):
        imr_0[extreme_mask] = -np.exp(
            stats.norm.logpdf(neg_eta[extreme_mask]) - stats.norm.logcdf(neg_eta[extreme_mask])
        )
...
class HeckmanSelectionImputer(BaseEstimator, TransformerMixin):
    """
    Heckman selection model imputer for single or multiple MNAR variables.

    Parameters
    ----------
    target_cols : Optional[List[str]], default=None
        Columns to impute using Heckman selection model. If None, imputes all columns with missing values.
    shadow_cols : Optional[Dict[str, str]], default=None
        Mapping of {target_col: shadow_var_col}. Shadow variables enter the selection equation W
        but are EXCLUDED from the outcome equation X (classic instrument / exclusion restriction).
    ...
    def __init__(
        ...
        self.shadow_cols = shadow_cols or {}
    ...
    def fit(self, X: Union[pd.DataFrame, np.ndarray], y=None):
        ...
        for col in df.columns:
            self.col_medians_[col] = float(df[col].median()) if df[col].notna().any() else 0.0
        ...
        for target in targets:
            ...
            if obs_mask.sum() < 10 or (~obs_mask).sum() < 5:
                continue
            ...
            shadow_var = self.shadow_cols.get(target)
            if shadow_var and shadow_var in covar_cols:
                # W contains shadow_var, X excludes shadow_var
                ...
                warnings.warn(
                    ...
                    "Identification relies purely on the Probit functional form.",
                    UserWarning,
                )
            ...
            try:
                ...
                # Fallback to logistic regression via statsmodels Logit
                logit_mod = sm.Logit(R, W_mat)
                logit_res = logit_mod.fit(disp=False, maxiter=100)
                gamma = logit_res.params / 1.6  # Scale logit to probit approximation

            # Compute linear predictor eta = W * gamma
            eta = W_mat @ gamma
    ...
    def transform(self, X: Union[pd.DataFrame, np.ndarray]) -> pd.DataFrame:
        ...
        for target, model_info in self.models_.items():
            ...
            # Predictor matrices for missing rows
            W_mis = df.loc[mis_mask, W_cols].fillna(df[W_cols].median()).to_numpy(dtype=float)
            W_mat_mis = sm.add_constant(W_mis, has_constant="add")
            eta_mis = W_mat_mis @ gamma

            # Compute lambda_0 for unobserved cases
            lambda_0 = _compute_imr_missing(eta_mis)

            X_mis = df.loc[mis_mask, X_cols].fillna(df[X_cols].median()).to_numpy(dtype=float)
            X_mat_mis = sm.add_constant(X_mis, has_constant="add")

            # Conditional expectation: E[Y | R=0] = X * beta + beta_lambda * lambda_0
            ...
            if self.stochastic:
                ...
                noise = rng.normal(0, np.sqrt(cond_var))
                expected_y += noise

            df.loc[mis_mask, target] = expected_y
